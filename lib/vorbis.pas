...
{$IF Defined(WIN32)}
  ...
  vorbisfilelib = 'libvorbisfile-3.dll';
...
{ vorbis_dsp_state buffers the current vorbis audio analysis/synthesis state.  The DSP state belongs to a specific logical bitstream }
  ...
  vorbis_dsp_state = packed record
    analysisp       : cint;
    vi              : pvorbis_info;
...
{ vorbis_block is a single block of data to be processed as part of
  ...
  alloc_chain = packed record
    ptr             : pointer;
    next            : palloc_chain;
  end;

  pvorbis_block = ^vorbis_block;
  vorbis_block = packed record
